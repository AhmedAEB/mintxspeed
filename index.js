...
let restartTimer;
function sendRequest(ws, pubkeys) {
...
function setupWebSocket() {
    const ws = new ws_1.default(`wss://atlas-mainnet.helius-rpc.com?api-key=8d4d870e-69ed-4675-9e1b-c5e02b1ef9e6`);
    ...
    ws.on('message', function incoming(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const isSwap = transaction.transaction.message.instructions.some((instruction) => instruction.programId === JUPITER_PROGRAM_ID || instruction.programId === RAYDIUM_PROGRAM_ID || instruction.programId === PUMP_PROGRAM_ID || instruction.programId === RAYDIUM_ROUTE_ID || instruction.programId === ORCA_PROGRAM_ID);
                if (!isSwap)
                    return; // Exit if no swap detected
                // Filter out swap instructions based on known Program IDs
                const swapInstructions = transaction.transaction.message.instructions.filter((instruction) => instruction.programId === JUPITER_PROGRAM_ID || instruction.programId === RAYDIUM_PROGRAM_ID || instruction.programId === PUMP_PROGRAM_ID || instruction.programId === RAYDIUM_ROUTE_ID || instruction.programId === ORCA_PROGRAM_ID);
                if (swapInstructions.length === 0)
                    return; // Exit if no swap instructions found
                ...
                const preBalances = (_b = transaction.meta.preTokenBalances) === null || _b === void 0 ? void 0 : _b.filter((balance) => balance.owner === transaction.transaction.message.accountKeys.find((key) => key.signer).pubkey);
                const postBalances = (_c = transaction.meta.postTokenBalances) === null || _c === void 0 ? void 0 : _c.filter((balance) => balance.owner === transaction.transaction.message.accountKeys.find((key) => key.signer).pubkey);
                ...
                const tokenChanges = yield Promise.all(preBalances.map((preBalance) => __awaiter(this, void 0, void 0, function* () {
                    const postBalance = postBalances.find((post) => post.mint === preBalance.mint);
                    if (!postBalance)
                        return null; // Skip if no matching post balance found
                    const change = postBalance.uiTokenAmount.uiAmount - preBalance.uiTokenAmount.uiAmount;
                    if (change === 0)
                        return null; // Skip if no change in balance
                    return {
                        change: change,
                ...
                if (!token1) {
                    ...
                    const newTokenMint = postBalances.find((postBalance) => !preBalances.some((preBalance) => preBalance.mint === postBalance.mint));
                    if (newTokenMint) {
                        token1 = {
